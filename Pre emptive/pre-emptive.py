...
def pre(newtask_Slots,task_Slots, total_timeR, processorsNum):
    cmp_tasks=[]
    ...
    hep = []
    for i in range(len(newtask_Slots)):
        if(newtask_Slots[i]!=cmp_tasks[i]):
            key = total_timeR/newtask_Slots[i]
            hep.append([key,i])
    heapq.heapify(hep)      #heapify the queue
...
#        queue = []
        current = []
        if(len(hep)>processorsNum):
            for num in range(processorsNum):
                current.append(heapq.heappop(hep))
        else:
            for num in range(len(hep)):
                current.append(heapq.heappop(hep))
        for k in current:
            cmp_tasks[k[1]] = cmp_tasks[k[1]] +1
            t = t+1
            if cmp_tasks[k[1]]!=newtask_Slots[k[1]]:
                key = ((cmp_tasks[k[1]]+1)*total_timeR)/newtask_Slots[k[1]]
                heapq.heappush(hep,[key,k[1]])
...
if __name__ == '__main__':
    ...
    while (taskNum < 101):
        ...
        while(repeat<20):
            ...
            for i in old1:
                tt = tt + int(i)
            miss = pre(old1, new1,tt, int(str_processorsNum))
